...
app.get('/start-exam', async (req, res) => {
  try {
    ...
    const randomQuestionIndex = Math.floor(Math.random() * randomExam.questions.length);
    const questionObj = randomExam.questions[randomQuestionIndex];
...
app.get('/get-question/:index', async (req, res) => {
  try {
    const exam = await Exam.findOne(); // Assuming only 1 exam exists
